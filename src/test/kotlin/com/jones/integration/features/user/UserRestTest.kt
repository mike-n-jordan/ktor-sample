...
class UserRestTest : BehaviorSpec({
    ...
    context("A user logging into an account and requesting their own data") {
        ...
        withTestApplication {
            val createUserUseCase = koin.get<CreateUserUseCase>()
            ...
            Given("A logged in user") {
                createUserUseCase.execute("user", "password".toCharArray(), UserType.USER)
                val loginRequest = client.submitForm(
                    url = application.href(UserResource.Login()),
                    formParameters = Parameters.build {
    ...
    context("An admin requesting all user data") {
        ...
        withTestApplication {
            ...
            Given("Logged in as an admin user AND there are users in the system") {
                createUserUseCase.execute("user1", "password".toCharArray(), UserType.USER)
                createUserUseCase.execute("user2", "password".toCharArray(), UserType.USER)
                val loginRequest = client.submitForm(
                    url = application.href(UserResource.Login()),
                    formParameters = Parameters.build {
                        append("username", "admin")
    ...
    context("An admin creating a new user") {
        ...
        withTestApplication {
            ...
            Given("A user who is not an admin") {
                createUserUseCase.execute("the_non_admin_user", "password".toCharArray(), UserType.USER)
                val loginRequest = client.submitForm(
                    url = application.href(UserResource.Login()),
                    formParameters = Parameters.build {
